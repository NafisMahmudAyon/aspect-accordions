...
document.addEventListener("DOMContentLoaded", () => {
	...
	accordionContainers.forEach((accordionContainer) => {
		const globalOptions = accordionContainer.dataset.accordionGlobalOptions
			? JSON.parse(accordionContainer.dataset.accordionGlobalOptions)
			: {};
		...
		const accordionItems = accordionContainer.querySelectorAll(
			".aspect-accordions-block-accordion-item"
		);

		console.log(accordionItems);

		accordionItems.forEach((accordionItem) => {
			const itemOptions = accordionItem.dataset.accordionItemOptions
				? JSON.parse(accordionItem.dataset.accordionItemOptions)
				: {};
			console.log(itemOptions);
			const header = accordionItem.querySelector(
				".aspect-accordions-block-accordion-header"
			);
			const content = accordionItem.querySelector(
				".aspect-accordions-block-accordion-content"
			);
			const icon = accordionItem.querySelector(
				".aspect-accordions-block-accordion-icon"
			);
